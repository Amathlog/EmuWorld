...
#include <common/utils/memoryVisitor.h>
#include <core/bus.h>
...
static std::unique_ptr<NesEmulator::Bus> s_bus;
static std::shared_ptr<NesEmulator::Controller> s_controller;

static std::array<uint32_t, NesEmulator::Cst::SCREEN_HEIGHT * NesEmulator::Cst::SCREEN_WIDTH> frame_buf;
...
void retro_init(void)
{
    ...
    s_bus = std::make_unique<NesEmulator::Bus>();
    s_bus->GetAPU().SetEnable(true);
    s_controller = std::make_shared<NesEmulator::Controller>();
    s_bus->ConnectController(s_controller, 0);
}
...
void retro_get_system_info(struct retro_system_info* info)
{
    ...
    info->library_name = "nes-emulator";
...
{
    info->geometry.base_width = NesEmulator::Cst::SCREEN_WIDTH;
    info->geometry.base_height = NesEmulator::Cst::SCREEN_HEIGHT;
    info->geometry.max_width = NesEmulator::Cst::SCREEN_WIDTH;
    info->geometry.max_height = NesEmulator::Cst::SCREEN_HEIGHT;
    info->geometry.aspect_ratio = 0.0f; // 0 = width/height
    info->timing.fps = 60;
    info->timing.sample_rate = NesEmulator::Cst::SAMPLE_RATE;
}
...
void retro_set_environment(retro_environment_t cb)
{
    ...
    static const struct retro_controller_description controllers[] = {
        {"NES", RETRO_DEVICE_SUBCLASS(RETRO_DEVICE_JOYPAD, 0)},
    };
...
{
    const uint8_t* screen = s_bus->GetPPU().GetScreen();

    for (size_t i = 0; i < frame_buf.size(); ++i)
    {
        frame_buf[i] = NesEmulator::Palette::GetRGBColor(screen[i]);
    }

    video_cb(frame_buf.data(), NesEmulator::Cst::SCREEN_WIDTH, NesEmulator::Cst::SCREEN_HEIGHT,
             NesEmulator::Cst::SCREEN_WIDTH * sizeof(uint32_t));
}
...
void retro_run(void)
{
    ...
    // NTSC
    constexpr int num_clocks_per_frame = 29780;
    for (int i = 0; i < num_clocks_per_frame; ++i)
    {
        if (s_bus->Clock())
        {
            video_callback();
...
bool retro_load_game(const struct retro_game_info* info)
{
    ...
    if (info && info->data)
    {
        Common::Utils::MemoryReadVisitor visitor(static_cast<const uint8_t*>(info->data), info->size);
