...
inline void GBCPaletteData::SerializeTo(Common::Utils::IWriteVisitor& visitor) const
{
    std::for_each(colors.cbegin(), colors.cend(), [&visitor](const auto& item) { visitor.WriteValue(item.data); });
}

inline void GBCPaletteData::DeserializeFrom(Common::Utils::IReadVisitor& visitor)
{
    ...
    if (m_scanlines <= 143)
    {
        ...
        if (m_lineDots < 80)
        {
            ...
            if (m_lineDots % 2 == 0 && m_lcdRegister.objEnable == 1 && m_selectedOAM.size() < 10)
            {
                ...
                // its size (8 or 16)
                if (m_scanlines + 16u >= entry.yPosition && m_scanlines + 16u < entry.yPosition + objSize)
                {
