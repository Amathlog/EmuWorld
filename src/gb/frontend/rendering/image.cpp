    ...
    , m_currentHeight(heightRes)
{
    ...
    m_imageBuffer.resize(m_internalResWidth * m_internalResHeight * 3);
}
...
void Image::UpdateGLTexture(bool bindBefore)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_internalResWidth, m_internalResHeight, GL_RGB, GL_UNSIGNED_BYTE,
                    m_imageBuffer.data());
...
void Image::UpdateInternalBuffer(const uint8_t* data, size_t size)
{
    ...
    std::memcpy(m_imageBuffer.data(), data, m_imageBuffer.size() * sizeof(*data));
...
{
    float vertices[] = {
        1.0f, 1.0f, 1.0f, 0.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 0.0f, 1.0f, -1.0f, 1.0f, 0.0f, 0.0f,
    };
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_internalResWidth, m_internalResHeight, 0, GL_RGB, GL_UNSIGNED_BYTE,
                 (void*)0);
